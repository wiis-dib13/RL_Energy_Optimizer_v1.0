...
public class DCNSFog {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// FIXED: Properly handle the ternary operator and casting
			if (CLOUD ) {
			...
			// After CloudSim.stopSimulation() in DCNSFog.java:
			// ===== AFTER SIMULATION =====
			...
			// You can choose what to optimize:
			// OPTION 1: totalEnergy (global)
			// OPTION 2: cloudEnergy (better for your case)

			// 👉 I recommend TOTAL ENERGY for RL:
			double energyToStore = totalEnergy;

			// cost removed (not reliable)
			double cost = 0;

			ModulePlacementMapping.recordSimulationResult(
			    ModulePlacementMapping.lastChosenAction,
			    energyToStore,
			    cost
			);
	...
	private static void createFogDevices(int userId, String appId) {
	    FogDevice cloud = CLOUD
	        ? createFogDevice("cloud", 44800, 40000, 100, 10000, 0, 0.01, 16*103, 16*83.25)
	        : createFogDevice("cloud", 44800, 40000, 100, 10000, 0, 0.01, 107.339, 83.4333);
	    cloud.setParentId(-1);

	    // Override power model to proxy-level when not in CLOUD mode
	    if (!CLOUD) {
	        org.cloudbus.cloudsim.power.PowerHost cloudHost =
	            (org.cloudbus.cloudsim.power.PowerHost) cloud.getHost();
	        cloudHost.setPowerModel(new org.fog.utils.FogLinearPowerModel(107.339, 83.4333));
	    }

	    fogDevices.add(cloud);

	    // ← THESE WERE MISSING — proxy + all areas/cameras
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		FogDevice router = createFogDevice("fog-"+id, 2800, 4000, 10000, 10000, 2, 0.0, 107.339, 83.4333);//i change 1 to 2 cause fog level is 2 after cloud and proxy
